...
def main():
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        time1 = time.time()
        train_acc, train_loss, MemBank = train(epoch, train_loader, model_s, model_t, criterion_cls, criterion_div,
                                               criterion_kd, optimizer, opt, MemBank)
        time2 = time.time()
...
def train(epoch, train_loader, model_s, model_t, criterion_cls, criterion_div, criterion_kd, optimizer, opt, MemBank):
    ...
    with tqdm(train_loader, total=len(train_loader)) as pbar:
        for idx, (input, input2, input3, input4, target, indices) in enumerate(pbar):
            ...
            # ===================forward=====================
            with torch.no_grad():
                (_, _, _, _, feat_t), (train_logit_t, eq_logit_t, inv_rep_t) = model_t[0](generated_data,
                                                                                          inductive=True)
            ...
            for ii in range(1, opt.trans):
                loss_inv += simple_contrstive_loss(inv_rep_0, inv_rep_s[(ii * batch_size):((ii + 1) * batch_size), :],
                                                   mn_arr, opt.contrast_temp)
            loss_inv = loss_inv / opt.trans

            loss = opt.w_ce * (opt.gamma * (loss_eq + loss_inv) + loss_ce) + opt.w_div * (
                    loss_div + loss_div_eq + loss_mse_inv + loss_mse_feat)
